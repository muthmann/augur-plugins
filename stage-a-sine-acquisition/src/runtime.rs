...
fn materialize_universal_protocol(name: &str) -> Result<Option<String>, String> {
    ...
    let path = std::env::temp_dir().join(format!("stage-a-universal-{name}.csv"));
...
impl<const A3: bool> SineAcquisition<A3> {
    ...
    fn begin_protocol(&mut self, context: &mut impl RecordingControl) {
        ...
        if A3 {
            ...
            {
                self.message = format!(
                    "A3 row {} ({}) records fewer than 5 cycles; increase duration_s or frequency_hz",
                    index + 1,
                    point.block
                );
                return;
        ...
        let controls_camera = self.camera_override.is_some()
            || plan.camera.is_some()
            || plan
                .points
                .iter()
        ...
        // A universal override starts from the host-confirmed current state;
        // the first camera reply below applies the requested fields. This
        // deliberately takes precedence over a legacy named profile in the
        // delegated CSV, otherwise the campaign would silently use the wrong
        // bias candidate.
        let camera_selection = if self.camera_override.is_some() {
            None
        } else {
            plan.camera.clone()
        };
        ...
        match phase {
            ...
            ProtocolPhase::AcquiringLease => {
                        ...
                        .is_some_and(|run| !run.firmware_prepared)
                {
                    ...
                    let request =
                        self.modulation_request(ModulationCommandV1::PrepareA1, &lease_id);
                    context.request_service(&request);
                ...
                match &reply.outcome {
                    HostCommandOutcome::CameraConfigurationApplied {
                        ...
                        if needs_override {
                            ...
                            self.protocol.as_mut().unwrap().camera_override_applied = true;
                            self.pending_camera_override = Some(snapshot);
                            return;
                        }
                        if let Some(run) = self.protocol.as_mut() {
...
impl<const A3: bool> Plugin for SineAcquisition<A3> {
    ...
    fn handle_service_request(
        ...
        let outcome = if request.service != stage_a_universal_runner::SERVICE_EXECUTE_BLOCK_V1 {
            ...
            PluginServiceOutcome::Rejected {
                ...
                message: "A1/A3 block execution requires the live worker".into(),
            }
        } else {
            match serde_json::from_value::<stage_a_universal_runner::ExecuteBlockRequest>(
                ...
                Ok(command) => PluginServiceOutcome::Rejected {
                    ...
                    message: format!("this runner cannot execute {:?}", command.experiment),
                },
    ...
    fn process_control(&mut self, context: &mut PluginControlContext<'_>) {
        ...
        if let Some(snapshot) = self.pending_camera_override.take() {
            let request_id = self.next_request_id();
            let _ = context.request_host(&HostCommandRequest {
                request_id,
                command: HostCommand::ApplyCameraConfiguration {
                    configuration: CameraConfigurationSourceV1::Snapshot { snapshot },
                },
...
"#;
    ...
    #[test]
    fn a_protocol_puts_a_controller_left_in_a2_back_into_a1_mode() {
        let folder = temp_folder("protocol-prepare-a1");
        let (mut plugin, _) = protocol_plugin(&folder, TWO_POINT_PROTOCOL);
        plugin.modulation.as_mut().unwrap().controller_mode = Some("A2".into());
        let mut sink = ControlSink::default();
        control_tick(&mut plugin, PluginControlInbox::default(), &mut sink);
        let lease_req = sink.services[0].request_id;
        sink.services.clear();
        ...
        control_tick(
            &mut plugin,
            inbox_with(vec![rejected(prepare_req, "STATE stop_before_config")]),
            &mut sink,
        );
        control_tick(&mut plugin, PluginControlInbox::default(), &mut sink);
        assert!(
    ...
    /// A controller already in A1 must not be reconfigured: `CONFIG` is refused
    /// while the acquisition runs, so the mode change would stop and restart
    /// the photodiode stream for nothing.
    #[test]
    fn a_controller_already_in_a1_is_left_alone() {
        let folder = temp_folder("protocol-prepare-skip");
        let (mut plugin, _) = protocol_plugin(&folder, TWO_POINT_PROTOCOL);
        let mut sink = ControlSink::default();
        control_tick(&mut plugin, PluginControlInbox::default(), &mut sink);
        let lease_req = sink.services[0].request_id;
...
1,2,fast,2000,2200,0,0,,,12.8,,,,,,ok,\n",
    ...
    fn a3_queries_complete_preparation_and_all_three_drive_commands() {
        ...
        assert!(originals.iter().all(|original| {
            !plugin
                .modulation_requests
                .iter()

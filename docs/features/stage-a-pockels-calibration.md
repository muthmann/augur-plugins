...
The fit is **never** applied automatically. `Apply to V_null / Vπ` refuses, with
the reason in the status line, while any of these hold:

| Gate | Reason |
|---|---|
| residual > 2 % of the detector span | the lobe model does not describe this sweep |
| lobe coverage < 0.75 | `Vπ` would be extrapolated, not measured |
| any clipped point | a clipped extremum hides where the extremum is |

Applying also re-validates the resulting drive: a calibration that cannot be
armed is rolled back rather than stored.

...
brightest code there puts maximum light at `u ≈ 0.5` with a null back at
`u = 1` (ADR 016). `u = 1` now holds exactly at `V_peak` by construction.
...
`V_peak=2490` gives DAC `2490` at `u=1` and DAC `1685` at `u=0.01`.
...
Under `CALIBRATED`, `V_null`/`V_peak`/`ū`/`a` define the operating band directly.
...
operating band / `ū` / `a`, to fit. Under `CALIBRATED` the DAC *floor* can no
...
are refused. It also pins the endpoint form: that `u` rises monotonically to
...
*distance* belongs is what peaked the light at `u = 0.5`.

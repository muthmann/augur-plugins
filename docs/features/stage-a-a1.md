...
final paths in the sidecar (ADR 012). The A1 output folder is therefore the only
...
**When something is wrong** (ADR 012):
...
boundary is A1's own pipeline restart and only the event fold resets (ADR 012).
...
Three of them guard the recording defects fixed in ADR 012: a photodiode without
a data directory is refused before any host command is sent; a photodiode failure

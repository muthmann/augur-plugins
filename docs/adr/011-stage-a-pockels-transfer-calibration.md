...
### 7. Review before apply, and restore always

The fit is never applied automatically; a second explicit press applies it, and
only when the residual is ≤ 2 % of the detector span, the sweep covered ≥ 0.75
of a lobe, and no point clipped. Applying re-validates the resulting drive and
rolls back if it cannot be armed. The sweep restores the pre-sweep drive on
every exit path, and refuses to run while a lease or protocol owns the DAC.

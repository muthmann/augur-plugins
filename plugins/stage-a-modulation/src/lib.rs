...
impl StageAModulationPlugin {
    ...
    fn optical_drive_state(&self) -> Option<OpticalDriveStateV1> {
        ...
        Some(OpticalDriveStateV1 {
            ...
            v_pi_dac: u16::try_from(drive.inversion.v_pi_dac.round() as i64).ok()?,
        })
    }
                ...
                modulation_items.push(SettingItem {
                    ...
                    tooltip: Some(
                        "DAC code where excitation light is brightest, on the same lobe as \
                         ...
                         wave Vπ = |V_peak − V_null| is derived, never typed. u = 1 holds \
                ...
                SettingsSection {
                    ...
                    items: vec![
                        ...
                        SettingItem {
                            ...
                            tooltip: Some(
                                "Writes the reviewed fit into the two observed endpoint settings. \
                                 Residual, dropped points, hysteresis, and clipping remain visible \
                                 as warnings; applying only refuses a lobe that cannot produce a \
                                 legal drive."
                                    .into(),
                            ),
                            kind: SettingKind::Button {
        ...
        if self.method == DriveMethod::Calibrated || self.mode.optical_target().is_some() {
            ...
            entries.push(StatusEntry::Text(match self.resolved_lobe() {
                Ok(lobe) => {
                    ...
                    format!(
                        "Lobe: Vπ = {:.0} codes — u 0 → {:.0} (min light), 0.5 → {:.0}, \
                         1 → {:.0} (max light){}",
                        inversion.v_pi_dac,

...
/// Refuse to apply a fit whose residual exceeds this fraction of the detector
/// span: past here the lobe model does not describe the measurement.
const MAX_APPLY_QUALITY: f64 = 0.02;
...
impl StageAModulationPlugin {
    ...
    fn complete_calibration_sweep(&mut self) {
        ...
        match calibration::fit_transfer(&points, max_code, self.detector_geometry) {
            Ok(fit) => {
                let status = format!(
                    "V_null {:.0}  Vπ {:.0}  residual {:.1}%{}  span {:.3} V ({})",
                    fit.v_null_dac,
                    fit.v_pi_dac,
                    fit.quality * 100.0,
                    fit.hysteresis
    ...
    fn apply_blocker(&self) -> Option<String> {
        ...
        if fit.quality > MAX_APPLY_QUALITY {
            return Some(format!(
                "residual {:.1}% exceeds {:.0}%: the lobe model does not describe this sweep",
                fit.quality * 100.0,
                MAX_APPLY_QUALITY * 100.0
            ));
        }
        if fit.lobe_coverage < MIN_APPLY_COVERAGE {
            return Some(format!(
                "the sweep covered only {:.0}% of a lobe; Vπ would be extrapolated",
                fit.lobe_coverage * 100.0
            ));
        }
        // A clipped extremum hides where the extremum actually is, which is
        // exactly what the fit is trying to locate.
        if fit.points.iter().any(|point| point.clipped) {
            return Some(
                "the detector clipped during the sweep; add attenuation and re-measure".into(),
            );
        }
        None
    }
...
"#;
    ...
    #[test]
    fn a_poor_or_clipped_fit_cannot_be_applied() {
        let mut plugin = live_plugin();
        let clipped_points: Vec<calibration::SweepPoint> =
            calibration::sweep_codes(4_095, 49, false)
                .into_iter()
                .map(|(code, direction)| {
                    let u = (std::f64::consts::PI * (f64::from(code) - 300.0) / 3_200.0)
                        .sin()
                        .powi(2);
        ...
        let blocker = plugin.apply_blocker().expect("clipping blocks the apply");
        assert!(blocker.contains("clipped"), "{blocker}");

        let before = (plugin.v_null_dac, plugin.v_pi_dac);
        plugin.set_setting("calibrate_apply", json!(true)).unwrap();
        assert_eq!((plugin.v_null_dac, plugin.v_pi_dac), before);
        assert!(plugin.calibration_id.is_none());
    }

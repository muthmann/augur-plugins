...
/// Refuse to apply a fit from a sweep that covered less of a lobe than this;
/// below it the quarter wave is extrapolated rather than measured.
const MIN_APPLY_COVERAGE: f64 = 0.75;
...
impl StageAModulationPlugin {
    ...
    /// Why the current fit must not be applied, if it must not.
    ///
    /// Only one thing is fatal: a sweep that never saw a full lobe cannot
    /// *measure* `Vπ`, it can only extrapolate one, so applying it would store
    /// a fabricated number. Everything else — residual, hysteresis, clipping —
    /// is a judgement the operator can make from the transfer-curve plot, and
    /// is reported as a warning instead of a block. A stray point can push the
    /// residual past any threshold while `Vπ` stays accurate to a few codes, so
    /// blocking on it withholds a good calibration for a bad reason.
    fn apply_blocker(&self) -> Option<String> {
        let fit = self.fit.as_ref()?;
        if fit.lobe_coverage < MIN_APPLY_COVERAGE {
            return Some(format!(
                "the sweep covered only {:.0}% of a lobe, so Vπ would be extrapolated rather \
                 than measured; widen the max limit or check the drive gain",
                fit.lobe_coverage * 100.0
            ));
        }
        None
    }
    ...
    fn apply_calibration_fit(&mut self) {
        ...
        if let Some(blocker) = self.apply_blocker() {
            self.calibration_status = format!("not applied: {blocker}");
            return;
        }
...
impl Plugin for StageAModulationPlugin {
    ...
    fn status_entries(&self) -> Vec<StatusEntry> {
        ...
        if let Some(fit) = self.fit.as_ref() {
            ...
            if let Some(blocker) = self.apply_blocker() {
                entries.push(StatusEntry::Text(format!("Cannot apply: {blocker}")));
            }
        }
...
"#;
    ...
    fn sweep_recovers_a_synthetic_lobe_and_restores_the_armed_drive() {
        ...
        assert!(
            plugin.apply_blocker().is_none(),
            "{:?}",
            plugin.apply_blocker()
        );
    ...
    /// A poor residual is the operator's call, made against the plot. Only a
    /// sweep that never saw a full lobe is fatal, because then the quarter wave
    /// would be extrapolated rather than measured.
    #[test]
    fn only_an_extrapolated_lobe_blocks_the_apply() {
        let mut plugin = live_plugin();

        // Clipped and scattered: warned about, still applicable.
        ...
        assert!(plugin.apply_blocker().is_none());
        ...
        // Less than three quarters of a lobe: refused, nothing written.
        let mut narrow = live_plugin();
        narrow.fit = Some(synthetic_fit(-0.090, 900, |_, _| {}));
        let blocker = narrow.apply_blocker().expect("extrapolated Vpi blocks");
        assert!(blocker.contains("extrapolated"), "{blocker}");
        let before = (narrow.v_null_dac, narrow.v_pi_dac);
        narrow.set_setting("calibrate_apply", json!(true)).unwrap();
        assert_eq!((narrow.v_null_dac, narrow.v_pi_dac), before);
        assert!(narrow.calibration_id.is_none());
    }

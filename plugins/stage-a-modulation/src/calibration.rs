...
pub fn fit_transfer(
    ...
    let point_spacing = swept_span / profile.len().max(2) as f64;

    // Scan the quarter wave over every period the sweep could resolve: from
    // four samples per lobe (below that the lobe is aliased) out to a lobe
    // twice the swept span (a barely-curved arc). Log-spaced, because a fixed
    // step wastes resolution at long periods and misses short ones.
    ...
    let (_, coarse_w) = best.ok_or(FitError::NoModulation)?;
    ...
    let rms = (harmonic.sse / points.len() as f64).sqrt();
    Ok(TransferFit {
        v_null_dac: v_null,
        v_pi_dac: w,
        offset_volts: p0,
...
mod tests {
    ...
    #[test]
    fn probe_small_signal_quality() {
        // Reject port, detector max 0.098 V at the null, as the bench reports.
        let lobe = LobeInversion { v_null_dac: 300.0, v_pi_dac: 860.0 };
        let make = |span: f64, noise_mv: f64, drift_mv: f64, hyst_mv: f64| {
            let codes = sweep_codes(4_095, 49, true);
            let n = codes.len();
            codes.into_iter().enumerate().map(|(i, (code, direction))| {
                let u = lobe.u_for_dac(f64::from(code));
                // deterministic pseudo-noise
                let wob = ((i as f64 * 12.9898).sin() * 43758.5453).fract() - 0.5;
                let drift = drift_mv * 1e-3 * (i as f64 / n as f64);
                let h = if direction == Direction::Descending { hyst_mv * 1e-3 } else { 0.0 };
                SweepPoint {
                    code, direction,
                    volts: 0.098 + span * u + wob * noise_mv * 1e-3 + drift + h,
                    peak_to_peak_volts: 0.001, clipped: false,
                }
            }).collect::<Vec<_>>()
        };
        for (label, pts) in [
            ("clean, span 90mV", make(-0.090, 0.0, 0.0, 0.0)),
            ("noise 2mV", make(-0.090, 2.0, 0.0, 0.0)),
            ("noise 5mV", make(-0.090, 5.0, 0.0, 0.0)),
            ("drift 10mV over sweep", make(-0.090, 1.0, 10.0, 0.0)),
            ("hysteresis 10mV", make(-0.090, 1.0, 0.0, 10.0)),
            ("ADC quantised 0.8mV", make(-0.090, 0.8, 0.0, 0.0)),
        ] {
            match fit_transfer(&pts, 4_095.0, DetectorGeometry::RejectedComplement) {
                Ok(f) => eprintln!("{label:26} -> Vpi {:7.1}  V_null {:7.1}  quality {:5.1}%  hyst {:?}",
                    f.v_pi_dac, f.v_null_dac, f.quality*100.0,
                    f.hysteresis.map(|h| format!("{:.1}%", h*100.0))),
                Err(e) => eprintln!("{label:26} -> ERR {e}"),
            }
        }
    }

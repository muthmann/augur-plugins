...
The optical load is an Excelitas **LM 0202** (`84502049000`), four-crystal
KD*P, 3×3 mm, 400–850 nm, 5 W.
...
- **Drive method** selects how the DAC operating band is defined:
  ...
  - `CALIBRATED`: `V_null`, `Vπ`, normalized cycle mean `ū`, and optical depth `a` determine the endpoints.
...
| `OPTICAL_LOG_SINE` | optical log-sine across the band | mean `ū`, converted to `u_g=ū/I_0(a/2)` |
| `OPTICAL_LINEAR_SINE` | optical linear-sine across the band | centre/mean `u_c=ū` |

Manual optical modes reuse the stored `V_null`/`Vπ` lobe and derive their effective `(u, a)`
...
2. Set **Detector port**. Stage-A watches the PBS *reject* port, where the detector is
   **brightest** at `V_null` — the default. This cannot be inferred from the sweep: a bright and
   ...
   excitation. Getting it wrong puts `V_null` one half-wave-voltage span out.
...
4. Read the result in the **Pockels transfer curve** view and the status line, then press
   **Apply to V_null / Vπ**. Anything questionable — a high residual, dropped points,

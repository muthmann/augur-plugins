...
impl Plugin for StageAA5Plugin {
    fn name(&self) -> &'static str { "Stage-A A5 Refractory and Load" }
    fn description(&self) -> &'static str { "Runs A5 rate, polarity and load protocols with coordinated camera RAW and PDQ acquisition." }
    fn enabled(&self) -> bool { self.inner.enabled() }
    fn set_enabled(&mut self, enabled: bool) { self.inner.set_enabled(enabled); }
    fn set_runtime_role(&mut self, role: PluginRuntimeRole) { self.inner.set_runtime_role(role); }
    fn reset(&mut self) { self.inner.reset(); }
    fn on_discontinuity(&mut self, reason: PluginDiscontinuity) { self.inner.on_discontinuity(reason); }
    ...
    fn handle_service_request(&mut self, request: &PluginServiceRequest, execution: &ExecutionContext) -> PluginServiceReply {
        let outcome = if request.service != SERVICE_EXECUTE_BLOCK_V1 {
            ...
            PluginServiceOutcome::Rejected { code: "effects_not_allowed".into(), message: "A5 block execution requires the live worker".into() }
        } else {
            match serde_json::from_value::<ExecuteBlockRequest>(request.payload.clone()) {
                Ok(command) if command.experiment == Experiment::A5 => {
                    ...
                    if let Err(error) = self.inner.set_setting("protocol_path", json!(protocol_path)) {
                        ...
                        PluginServiceOutcome::Accepted { payload: json!({"state":"started"}) }
                    }
                }
                Ok(command) => PluginServiceOutcome::Rejected { code: "wrong_target".into(), message: format!("A5 cannot execute {:?}", command.experiment) },
    ...
    fn settings_schema(&self) -> SettingsSchema { self.inner.settings_schema() }
    fn get_setting(&self, key: &str) -> Option<Value> { self.inner.get_setting(key) }
    fn set_setting(&mut self, key: &str, value: Value) -> Result<(), String> { self.inner.set_setting(key, value) }
    fn status_entries(&self) -> Vec<StatusEntry> {
        let mut status = self.inner.status_entries();
        status.insert(0, StatusEntry::Text("A5 analysis mode: refractory/load; timing conclusions remain offline".into()));
        status
    }
}
